/** 
 *FileName:     DoubleRowEliminateLogic.cs 
 *Author:       HeJunJie 
 *Version:      1.0 
...
namespace Game.Match3
{
    ...
    /// 双行特效消除
    /// </summary>
    public class DoubleRowEliminateLogic : EliminateBase
    {
        public override void ProcessEliminateLogic(params object[] args)
        {
            ...
            if (isTransverse)
            {
                List<M3Item> item1List = M3GameManager.Instance.GetLineItem(x1, y1, true, true);
                List<M3Item> item2List = M3GameManager.Instance.GetLineItem(x2, y2, true, true);
            ...
            else
            {
                ...
                item1.crushScore = SpecialEliminateScore.DoubleLine;
                Vector3 tmp = M3Supporter.Instance.GetItemPositionByGrid(x1, y1);
                Vector3 tmp2 = M3Supporter.Instance.GetItemPositionByGrid(x2, x2);
                if (!M3GameManager.Instance.isAutoAI)
                {
                    M3FxManager.Instance.FireArrow(tmp, false);

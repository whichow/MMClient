...
//[AddComponentMenu("UI/KUIGrid", 37)]
[SelectionBase]
[ExecuteInEditMode]
...
public class KUIGrid : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
{
    ...
    {
        return uiPool ? uiPool.CreateItem(index) : null;
    }
    ...
    protected float contentSpacing
    {
        get
        {
            ...
            if (content != null)
            {
                ...
                if (layoutGroup != null)
                {
                    m_ContentSpacing = layoutGroup.spacing;
                }
    ...
    public void RefillItemsFromEnd(int offset = 0)
    {
        ...
        _endItemIndex = totalCount - offset;
    ...
    {
        if (totalCount >= 0 && _beginItemIndex < contentConstraintCount)
        {
            return 0;
        }
    ...
    protected float DeleteItemAtBegin()
    {
        ...
        if (((m_Dragging || m_Velocity != Vector2.zero) && _endItemIndex >= totalCount - 1) || content.childCount == 0)
        {
            return 0;
        }
        ...
        for (int i = 0; i < contentConstraintCount; i++)
        {
            var retItem = content.GetChild(0) as RectTransform;
            size = Mathf.Max(GetItemSize(retItem), size);
            ReturnItem(retItem);
    ...
    {
        if (totalCount >= 0 && _endItemIndex >= totalCount)
        {
            return 0;
        }
        ...
        {
            var newItem = CreateItem(_endItemIndex++);
            size = Mathf.Max(GetItemSize(newItem), size);
    ...
    {
        if (((m_Dragging || m_Velocity != Vector2.zero) && totalCount >= 0 && _beginItemIndex < contentConstraintCount)
        ...
        for (int i = 0; i < contentConstraintCount; i++)
        {
            var retItem = content.GetChild(content.childCount - 1) as RectTransform;
            size = Mathf.Max(GetItemSize(retItem), size);
            ReturnItem(retItem);
    ...
    {
        var vMin = viewBounds.min;
        var vMax = viewBounds.max;
        var cMin = contentBounds.min;
        var cMax = contentBounds.max;

        var result = false;

        if (this.vertical)
        {
            if (vMin.y > cMin.y + threshold)
            {
            ...
            else if (vMin.y < cMin.y)
            {
                float size = CreateItemAtEnd(), totalSize = size;
            ...
            if (vMax.y < cMax.y - threshold)
            {
                float size = DeleteItemAtBegin(), totalSize = size;
            ...
            else if (vMax.y > cMax.y)
            {
                float size = CreateItemAtBegin(), totalSize = size;
        ...
        get
        {
            ...
            if (content != null)
            {
                ...
                if (m_GridLayout != null)
                {
                    ...
                    {
                        //Debug.LogWarning("Flexible not supported yet");
                    }
    ...
    public float horizontalNormalizedPosition
    {
        get
        {
            ...
            if (totalCount > 0 && _endItemIndex > _beginItemIndex)
            {
                //TODO: 如果有空格 有一定的误差
    ...
    protected void UpdateBounds()
    {
        ...
        //    m_ContentBounds = GetBounds();
        //}
        _needUpdate = UpdateItems(m_ViewBounds, m_ContentBounds);
        // ============ScrollRect Modify============
    ...
    private void UpdateScrollbars(Vector2 offset)
    {
        if (m_HorizontalScrollbar)
        {
            ...
            {
                m_HorizontalScrollbar.size = Mathf.Clamp01((m_ViewBounds.size.x - Mathf.Abs(offset.x)) / m_ContentBounds.size.x * (_endItemIndex - _beginItemIndex) / totalCount);
            }
            else
            {
        ...
        if (m_VerticalScrollbar)
        {
            ...
            {
                m_VerticalScrollbar.size = Mathf.Clamp01((m_ViewBounds.size.y - Mathf.Abs(offset.y)) / m_ContentBounds.size.y * (_endItemIndex - _beginItemIndex) / totalCount);
            }
            else
            {
